...
*Naming note:* this Crit resource is called **"Flourish"** in player-facing materials; use **"Crit"**
in design and card text. (Keyword formatting gospel → `writing-conventions.md`.)
...
| **[Crit]** | The tier above Success. Unlocks spend options on the played card. Spending does not remove the die; it only sets which options are available. Spend after all dice are fully resolved. *(Player-facing name: **Flourish**.)* |
...
- [Crit] options are declared after all dice are fully resolved; spending a Crit does not remove the
  die from the pool.
