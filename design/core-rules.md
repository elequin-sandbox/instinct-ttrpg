...
- **Count your Crits** — how many dice across the whole pool still show a **6**. That count is how many
  **Flourishes** (spend options on the played card) you may take.
- **Read vs. the opposing pool** — player dice clear opposing dice one at a time. No hidden DC or
  target number; the GM's pool *is* the difficulty.
...
| **[Crit]** | A natural **6** on a die (explodes once). After resolution, count Crits — each one may be spent to take one **Flourish** option from the played card. Spending does not remove the die from the pool. |
| **Flourish** | A bonus spend line on an ability card (not a die result). Costs 1 Crit per Flourish taken unless the card says otherwise. Listed under the card's Crit/Flourish block. |
...
— write a party member's name on it and set it aside (it shuffles into *their* deck, not yours);
...
*Not in the 18:* Connection (other players' decks), Item + Core (Active area). Instinct draft pool: 30
options at creation. *Hypergeometric note:* 5 Instincts in 18 cards → expect an Instinct in most hands.
